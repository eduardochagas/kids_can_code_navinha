...
class Player(pygame.sprite.Sprite):
    ...
    def shoot(self):
        bullet = Bullet(self.rect.centerx, self.rect.top) # insere o centerx do player no centerx do bullet e insere o top do player no bottom do bullet
...
class Bullet(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.rect.bottom < 0:
            self.kill() # esse método é do módulo pygame.sprite.Sprite.kill() da documentação Pygame.
...
bullets = pygame.sprite.Group() # define um grupo somente para as balas (bullets)
...
while running:
    ...
    if hits: # se houver colisão, execute a linha identada
        running = False
